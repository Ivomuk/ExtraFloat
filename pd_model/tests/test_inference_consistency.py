...
def _transform_with_fitted(df: pd.DataFrame, fitted_params, pd_features, log_cols, cap_cols, signed_log_cols) -> pd.DataFrame:
...
def test_transform_report_contains_lo_hi_bounds():
    ...
    winsorized_actions = {
        "forced_signed_log_ok", "signed_log_ok", "log_cap_ok",
        "cap_ok", "cap_only_due_to_negatives",
        "forced_signed_log_reverted_raw_cap", "signed_log_reverted_raw_cap",
...
def test_single_row_score_equals_batch_row():
    ...
    batch_result = _transform_with_fitted(batch_df, fitted_params, pd_features, log_cols, cap_cols, signed_log_cols)
    single_result = _transform_with_fitted(single_df, fitted_params, pd_features, log_cols, cap_cols, signed_log_cols)
...
def test_row_order_independence():
    ...
    result_orig = _transform_with_fitted(scoring_df, fitted_params, pd_features, log_cols, cap_cols, signed_log_cols)
    result_shuf = _transform_with_fitted(shuffled_df, fitted_params, pd_features, log_cols, cap_cols, signed_log_cols)
...
def test_duplicate_extreme_rows_do_not_pollute():
    ...
    polluted_df = pd.concat(
        [normal_df] + [extreme_row] * 500, ignore_index=True
    )

    result_clean = _transform_with_fitted(normal_df, fitted_params, pd_features, log_cols, cap_cols, signed_log_cols)
    result_polluted = _transform_with_fitted(polluted_df, fitted_params, pd_features, log_cols, cap_cols, signed_log_cols)
    ...
    for col in feature_cols:
        ...
        np.testing.assert_allclose(
            clean_vals, polluted_normal_vals, atol=1e-5,
...
def test_deterministic_across_repeated_executions():
    ...
    result_1 = _transform_with_fitted(scoring_df, fitted_params, pd_features, log_cols, cap_cols, signed_log_cols)
    result_2 = _transform_with_fitted(scoring_df, fitted_params, pd_features, log_cols, cap_cols, signed_log_cols)
    ...
    for col in feature_cols:
        ...
        np.testing.assert_array_equal(
            result_1[col].values, result_2[col].values,
