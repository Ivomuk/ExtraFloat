...
class TestRunStressTest:
    ...
    def test_required_columns(self):
        scored = _make_scored_df()
        thresh = _make_policy_tbl()
        result = run_stress_test(scored, thresh, operating_points=(0.20,), stress_multipliers=(1.0, 2.0))
        for col in (
            "operating_point",
            "stress_multiplier",
            "locked_pd_cutoff",
...
class TestBuildStressSummary:
    def test_wide_format(self):
        scored = _make_scored_df()
        thresh = _make_policy_tbl()
        result = run_stress_test(scored, thresh, operating_points=(0.20, 0.50), stress_multipliers=(1.0, 2.0))
