...
def _get_drift_config(config: dict[str, Any] | None) -> dict[str, Any]:
...
class FeatureDriftResult:
    ...
    percentile_shift: dict[str, float] | None
...
class CompositionDriftResult:
    ...
    cramers_v: float | None  # bias-corrected; None when scipy absent
...
class DriftReport:
    ...
    def summary_dict(self) -> dict[str, Any]:
        ...
        return {
            ...
            "top_composition_alerts": [
                r.feature for r in self.composition_drift if r.severity == SEVERITY_ALERT
            ],
...
def _monitor_categorical(
    ...
    chi2, pval, sig = _compute_chi2(
        ref_counts, cur_counts, comp_cfg["chi2_alpha"], comp_cfg["min_expected_freq"]
    )
    ...
    if sig:
        ...
        severity = (
            ...
            else (SEVERITY_MONITOR if max_abs_shift >= comp_cfg["alert_fraction_delta"] else SEVERITY_STABLE)
        )
    else:
        severity = SEVERITY_STABLE
...
def _health_result(
    ...
    cur_rate: float,
    *,
    absolute_alert: float | None = None,
    cur_rate_ceiling: float | None = None,
    relative_monitor: float | None = None,
...
def _detect_silent_recalibration(
    ...
    shift = abs(result.ref_distribution.get(ref_top, 0.0) - result.cur_distribution.get(ref_top, 0.0))
