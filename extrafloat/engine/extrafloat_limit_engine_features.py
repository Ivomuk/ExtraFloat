...
def prepare_borrower_limit_features(
    ...
    """
    logger.info("prepare_borrower_limit_features: input rows = %d", len(borrower_limit_df))

    _check_required_columns(borrower_limit_df, BORROWER_LIMIT_REQUIRED_COLUMNS, "borrower_limit_features")
    ...
    df["stability_proxy"] = df["prior_on_time_streak"].fillna(0) - df["prior_default_streak"].fillna(0)

    logger.info("prepare_borrower_limit_features: output rows = %d", len(df))
    return df
...
def prepare_loan_summary_recent_features(
    ...
    _check_required_columns(loan_summary_df, LOAN_SUMMARY_REQUIRED_COLUMNS, "loan_summary_recent_features")
    ...
    df["credit_days_since_last_repayment"] = (df["snapshot_dt"] - df["last_repayment_date"]).dt.days.clip(
        lower=0
    )

    df["recent_credit_active_flag"] = ((df["disbursement_vol_1m"] > 0) | (df["repayment_vol_1m"] > 0)).astype(
        int
    )
    ...
    logger.info("prepare_loan_summary_recent_features: output rows = %d", len(df))
    return df
...
def build_extrafloat_limit_engine_features(
    ...
    if "agent_tier_ceiling_multiplier" in merged.columns:
        ...
        merged["agent_tier_ceiling_multiplier"] = merged["agent_tier_ceiling_multiplier"].fillna(
            _tier_default
        )
    ...
    merged["recent_repayment_strength"] = 0.70 * _col(merged, "repayment_coverage_1m") + 0.30 * _col(
        merged, "repayment_count_coverage_1m"
    )
    ...
    merged["is_thin_file"] = (_col(merged, "total_loans") < _thin_file_threshold).astype(int)
