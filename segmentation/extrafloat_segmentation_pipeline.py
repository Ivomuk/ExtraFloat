...
DEFAULT_CLUSTERING_CONFIG: dict[str, Any] = {
    ...
    # When True (default), flag_anomalies' Stage 1 (HDBSCAN) runs on a UMAP
    # embedding (same as run_diagnostic_clustering) instead of raw PCA
    # space. Real numbers on the retail-filtered population settled this:
    # raw PCA space gave 30.0% is_global_anomaly with the default
    # hdbscan_min_cluster_size/hdbscan_min_samples — PCA space is simply
    # more diffuse, so far more points fail HDBSCAN's density criterion and
    # get dumped into noise, not because a third of agents are genuinely
    # anomalous — vs. 1.3% noise for the *same* settings on the UMAP
    # embedding, with a far more usable is_anomaly rate overall (2.5% vs.
    # 30.7%) and meaningfully differentiated per-cluster LOF rates instead
    # of one dominant cluster's local anomalies being swallowed by Stage 1's
    # bloated noise bucket. Set False to fall back to the cheaper raw-PCA
    # Stage 1 (skips UMAP's single-threaded runtime) if that cost isn't
    # acceptable on every production run — accuracy is what's traded away.
    "anomaly_hdbscan_use_umap": True,
...
def flag_anomalies(
    ...
    Stage 1 (HDBSCAN, dependency-graceful) runs alone — no GMM — on a UMAP
    embedding by default (`clustering.anomaly_hdbscan_use_umap`, default
    True; same embedding function `run_diagnostic_clustering` uses),
    flagging agents that don't belong to *any* dense cluster at all
    (`is_global_anomaly`). This runs on every production run by default
    ...
    False, which additionally runs its own GMM + UMAP pass and computes a
    separately-embedded anomaly signal, `diag_is_anomaly`).

    UMAP is the default here (not the cheaper raw PCA space used elsewhere
    in this module) because it materially changes what the flag means:
    measured on the retail-filtered population, raw PCA space produced
    30.0% `is_global_anomaly` (30.7% `is_anomaly` overall) with the
    identical HDBSCAN settings that gave 1.3% noise / 2.5% `is_anomaly` on
    the UMAP embedding — PCA space is
    simply more diffuse and fails HDBSCAN's density criterion far more
    often, not because roughly a third of agents are genuinely anomalous.
    A ~30% hit rate makes `is_anomaly` useless as a "flag this for review"
    signal; ~2.5% is usable. Set `anomaly_hdbscan_use_umap=False` to fall
    back to the cheaper raw-PCA Stage 1 (skips UMAP's single-threaded
    runtime) if that cost isn't acceptable on every production run —
    accuracy is what's traded away for it.
    ...
    *auxiliary* signal would defeat that.
    ...
    if cfg.get("anomaly_hdbscan_use_umap", True):
        try:
            ...
            logger.error(
                "flag_anomalies: UMAP embedding failed (%s) — falling back to "
                "raw PCA space for Stage 1 this run. is_global_anomaly may be "
                "noisier than usual (see anomaly_hdbscan_use_umap docs).",
                exc,
            )
