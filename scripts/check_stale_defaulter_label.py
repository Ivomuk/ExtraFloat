...
check_post_snapshot_disbursement.py:
  - the distribution of days_since_last_repayment (days between the last
    actual repayment event and the July-31 snapshot).
  - days between latest_state_date and --blacklist-date -- a direct "how
    long before being blacklisted did this loan's tracked state last
    update" measure.

NOTE: these columns live in the loan-history-snapshot CSV itself (built by
data/loan_history_snapshot_query.txt) -- wl_bl_eval_matched_agents.csv only
carries a curated subset of the engine's output columns and does not
include them, so --loan-history-file is required and gets joined in by
msisdn.
...
def main():
    ...
    ap.add_argument("--loan-history-file", required=True,
                     help="The loan-history-snapshot CSV built by data/loan_history_snapshot_query.txt "
                          "(has days_since_last_repayment / latest_state_date -- not in --matched-file)")
    ...
    if "days_since_last_repayment" not in matched_lh.columns:
        print(f"NOTE: 'days_since_last_repayment' not found in {lh_path}.")
        print(f"Columns present: {list(lh_df.columns)}")
    else:
        dslr = pd.to_numeric(matched_lh["days_since_last_repayment"], errors="coerce")
        n_non_null = dslr.notna().sum()
        print("=" * 78)
        print(f"days_since_last_repayment (days between last repayment and the "
              f"snapshot date) -- n_non_null={n_non_null:,} / {len(matched_lh):,}")
        print("=" * 78)
        if n_non_null > 0:
    ...
    if "latest_state_date" not in matched_lh.columns:
        print(f"NOTE: 'latest_state_date' not found in {lh_path} -- skipping that section.")
    else:
        state_date = pd.to_datetime(matched_lh["latest_state_date"], errors="coerce")
        ...
        print("=" * 78)
        print(f"Days between latest_state_date (last tracked loan-state update) "
